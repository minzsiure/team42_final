...
def request_handler(request):
    if request["method"] == "POST":
        ...
        with sqlite3.connect(ht_db) as c:
            ...
            routes = c.execute(
                '''SELECT route_building_names, route_center_coordinates, route_building_id FROM route_table WHERE route_id = ?;''', (search_id,)).fetchall()
            result = {}
            result["building_names"] = eval(routes[0][0])
            result["route_center_coordinates"] = eval(routes[0][1])
            result["route_building_id"] = eval(routes[0][2])
